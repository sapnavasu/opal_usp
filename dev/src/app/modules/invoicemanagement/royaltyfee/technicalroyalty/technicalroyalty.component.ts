...
@Component({
  selector: 'app-technicalroyalty',
  templateUrl: './technicalroyalty.component.html',
  styleUrls: ['./technicalroyalty.component.scss'],
...
export class TechnicalroyaltyComponent implements OnInit {
  ...
  Export() {
    ...
    this.service.export(this.page, this.paginator?.pageIndex, this.filterdata, this.filterDataPage, showCol).subscribe((data: any) => {
  ...
  download(id: any) {
    this.service.download(id).subscribe((data: any) => {
      let response = data.data.attend;
      var link = document.createElement('a');
      link.href = response
      link.click();
  ...
  viewTechRoyalty(id: any) {
    console.log("ID", id);
    this.route.navigate(['/invoicemanagement/rayoltytechfee/'], { queryParams: { roy_pk: btoa(id) } });
  }
  ...
  receviceDateValue(formData: any) {
    this.service.techGenerateinvoice(moment(formData._d)?.format('YYYY-MM')).subscribe((data: any) => {
      if (data.data.status) {
        this.toastr.success(data.data.message);
        // window.location.reload();
      } else {
  ...
  getData(limit: any, index: any, searchkey: any) {
    ...
    this.service.getTechRolayFeeList(limit, index, searchkey, this.filterDataPage).subscribe((data: any) => {
