...
export class GridlistComponent implements OnInit {
 ...
 status1;
  index = 0;
  dataSource : MatTableDataSource<requestfor>;
  stktype;
  isfocalpoint;
  useraccess;
  createaccess = false;
  ...
  displayedColumns = [  
    { def: "name",search: "row-one", label: "Request For", visible: true,disabled: false },
  ...
  ngOnInit(): void {
    ...
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][47] && this.useraccess[moduleid][47].create == 'Y'){
      this.createaccess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][47] && this.useraccess[moduleid][47].read == 'Y'){
      this.viewacess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][47] && this.useraccess[moduleid][47].update == 'Y'){
      this.updateaccess = true;
    }
    ...
    if (!this.viewacess) {
      ...
      // this.dataSource = new MatTableDataSource<requestfor>(this.mockData);
      this.getrequestfor(this.page,this.index, this.filterdata);
      // this.getinitialdata();
    }
  ...
  clearFilter(){
    ...
    $(".clear").trigger("click"); 
    this.dataSource = new MatTableDataSource<requestfor>([]);
    this.tblplaceholder = false;
    this.requestforlist_length = 0;
   this.getrequestfor(this.paginator.pageSize, this.paginator.pageIndex, this.filterdata)
  ...
  edit(id) {
    this.router.navigate(['/configuration/masterdataconfiguration/requestfor/edit/'+id]);
  }

  getrequestfor(limit,index,searchkey, sorting = null){
    this.tblplaceholder = true;
    this.service.getreferencelist(this.mastertype,limit,index,searchkey, sorting).subscribe(res=>{
      this.tblplaceholder = false;
      this.requestforlist = res.data.data.refer;
      this.requestforlist_length = res.data.data.totalcount;  
      this.dataSource = new MatTableDataSource<requestfor>(this.requestforlist);
  ...
  sorting(event,key){
    ...
    this.getrequestfor(this.page,this.index, this.filterdata, sorting);
  }
