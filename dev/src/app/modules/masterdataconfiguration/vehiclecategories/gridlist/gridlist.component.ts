...
export class GridlistComponent implements OnInit {
 ...
 status1;
  ...
  mockData =[{code:'LVI', vehicle_en:'Defensive driving', status:1, createdOn:'28-09-2023',
  createdBy:'Thomas',lastUpdatedOn:'28-09-2023',lastUpdatedBy:'Clare'}]
  displayedColumns = [  
    { def: "code",search: "row-one", label: "Course Category Code", visible: true,disabled: false },
    { def: "vehicle",search: "row-two", label: "Vehicle Category", visible: true ,disabled: false},
  ...
  ngOnInit(): void {
    ...
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][5] && this.useraccess[moduleid][5].create == 'Y'){
      this.createaccess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][5] && this.useraccess[moduleid][5].read == 'Y'){
      this.viewacess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][5] && this.useraccess[moduleid][5].update == 'Y'){
      this.updateaccess = true;
    }
    ...
    if (!this.viewacess) {
      ...
      // this.dataSource = new MatTableDataSource<vehiclecategory>(this.mockData);
      this.getvehicle(this.page,this.index, this.filterdata);
      // this.getinitialdata();
    }
  ...
  clearFilter(){
    ...
    $(".clear").trigger("click");     
    this.dataSource = new MatTableDataSource<vehiclecategory>([]);
    this.tblplaceholder = false;
    this.vehiclelist_length = 0;
   this.getvehicle(this.paginator.pageSize, this.paginator.pageIndex, this.filterdata)
  ...
  add() {
    this.router.navigate(['/configuration/masterdataconfiguration/vehiclecategories/add']);
  }


  edit(id) {
    this.router.navigate(['/configuration/masterdataconfiguration/vehiclecategories/edit/1']);
  }

  getvehicle(limit,index,searchkey, sorting = null){
    this.tblplaceholder = true;
    this.service.getvehiclelist(limit,index,searchkey, sorting).subscribe(res=>{
      this.tblplaceholder = false;
      this.vehiclelist = res.data.data.courses;
      this.vehiclelist_length = res.data.data.totalcount;  
      this.dataSource = new MatTableDataSource<vehiclecategory>(this.vehiclelist);
  ...
  sorting(event,key){
    ...
    this.getvehicle(this.page,this.index, this.filterdata, sorting);
  }
  ...
  updatestatus(id, status){
    ...
    this.service.updatvehiclestatus(data).subscribe(res=>{

      this.toastr.success(this.i18n('Vehicle Category status updated Successfully'), ''), {
        timeOut: 2000,
        closeButton: false,
      };
      this.getvehicle(this.page,this.index, this.filterdata);
