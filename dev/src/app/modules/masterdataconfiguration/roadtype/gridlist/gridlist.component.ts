...
export class GridlistComponent implements OnInit {
 ...
 status1;
  index = 0;
  dataSource : MatTableDataSource<roadtype>;
  stktype;
  isfocalpoint;
  useraccess;
  createaccess = false;
  ...
  displayedColumns = [  
    { def: "name",search: "row-one", label: "Road Type", visible: true,disabled: false },
  ...
  ngOnInit(): void {
    ...
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][49] && this.useraccess[moduleid][49].create == 'Y'){
      this.createaccess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][49] && this.useraccess[moduleid][49].read == 'Y'){
      this.viewacess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][49] && this.useraccess[moduleid][49].update == 'Y'){
      this.updateaccess = true;
    }
    ...
    if (!this.viewacess) {
      ...
      // this.dataSource = new MatTableDataSource<roadtype>(this.mockData);
      this.getroadtype(this.page,this.index, this.filterdata);
      // this.getinitialdata();
    }
  ...
  clearFilter(){
    ...
    $(".clear").trigger("click"); 
    this.dataSource = new MatTableDataSource<roadtype>([]);
    this.tblplaceholder = false;
    this.roadtypelist_length = 0;
   this.getroadtype(this.paginator.pageSize, this.paginator.pageIndex, this.filterdata)
  ...
  edit(id) {
    this.router.navigate(['/configuration/masterdataconfiguration/roadtype/edit/'+id]);
  }

  getroadtype(limit,index,searchkey, sorting = null){
    this.tblplaceholder = true;
    this.service.getreferencelist(this.mastertype,limit,index,searchkey, sorting).subscribe(res=>{
      this.tblplaceholder = false;
      this.roadtypelist = res.data.data.refer;
      this.roadtypelist_length = res.data.data.totalcount;  
      this.dataSource = new MatTableDataSource<roadtype>(this.roadtypelist);
  ...
  sorting(event,key){
    ...
    var sorting = {
      ...
      this.getroadtype(this.page,this.index, this.filterdata, sorting);
