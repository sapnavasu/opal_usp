...
export class GridlistComponent implements OnInit {
 ...
 status1;
  index = 0;
  dataSource : MatTableDataSource<Vehiclemanufacturer>;
  stktype;
  isfocalpoint;
  useraccess;
  createaccess = false;
  ...
  displayedColumns = [  
    { def: "name",search: "row-one", label: "Vehicle Manufacturer", visible: true,disabled: false },
  ...
  ngOnInit(): void {
    ...
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][52] && this.useraccess[moduleid][52].create == 'Y'){
      this.createaccess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][52] && this.useraccess[moduleid][52].read == 'Y'){
      this.viewacess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][52] && this.useraccess[moduleid][52].update == 'Y'){
      this.updateaccess = true;
    }
    ...
    if (!this.viewacess) {
      ...
      // this.dataSource = new MatTableDataSource<Vehiclemanufacturer>(this.mockData);
      this.getVehiclemanufacturer(this.page,this.index, this.filterdata);
      // this.getinitialdata();
    }
  ...
  clearFilter(){
    ...
    $(".clear").trigger("click");   
    this.dataSource = new MatTableDataSource<Vehiclemanufacturer>([]);
    this.tblplaceholder = false;
    this.Vehiclelist_length = 0;
   this.getVehiclemanufacturer(this.paginator.pageSize, this.paginator.pageIndex, this.filterdata)
  ...
  add() {
    this.router.navigate(['/configuration/masterdataconfiguration/vehiclemanufacturers/add']);
  }

  edit(id) {
    this.router.navigate(['/configuration/masterdataconfiguration/vehiclemanufacturers/edit/'+id]);
  }

  getVehiclemanufacturer(limit,index,searchkey, sorting = null){
    this.tblplaceholder = true;
    this.service.getreferencelist(this.mastertype,limit,index,searchkey, sorting).subscribe(res=>{
      this.tblplaceholder = false;
      this.Vehiclelist = res.data.data.refer;
      this.Vehiclelist_length = res.data.data.totalcount;  
      this.dataSource = new MatTableDataSource<Vehiclemanufacturer>(this.Vehiclelist);
  ...
  sorting(event,key){
    ...
    this.getVehiclemanufacturer(this.page,this.index, this.filterdata, sorting);
  }
  ...
  updatestatus(id, status){
    ...
    this.service.updatreferencestatus(data).subscribe(res=>{

      this.toastr.success(this.i18n('Vehical Manufacturer status updated Successfully'), ''), {
        timeOut: 2000,
        closeButton: false,
      };
      this.getVehiclemanufacturer(this.page,this.index, this.filterdata);
