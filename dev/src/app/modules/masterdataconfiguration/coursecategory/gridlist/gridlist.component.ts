...
export class GridlistComponent implements OnInit {
 ...
 status1;
  ...
  mockData =[{code:'LVI', course_en:'Defensive driving', status:1, createdOn:'28-09-2023',
  ...
  displayedColumns = [  
    ...
    { def: "course",search: "row-two", label: "Course Category", visible: true ,disabled: false},
  ...
  ngOnInit(): void {
    ...
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][42] && this.useraccess[moduleid][42].create == 'Y'){
      this.createaccess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][42] && this.useraccess[moduleid][42].read == 'Y'){
      this.viewacess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][42] && this.useraccess[moduleid][42].update == 'Y'){
      this.updateaccess = true;
    }
    ...
    if (!this.viewacess) {
      ...
      // this.dataSource = new MatTableDataSource<coursecategory>(this.mockData);
  ...
  clearFilter(){
    ...
    $(".clear").trigger("click");  
    this.dataSource = new MatTableDataSource<coursecategory>([]);
  ...
  add() {
    this.router.navigate(['/configuration/masterdataconfiguration/coursecategory/add']);
  }


  edit(id) {
    this.router.navigate(['/configuration/masterdataconfiguration/coursecategory/edit/'+id]);
  }

  getcourse(limit,index,searchkey, sorting = null){
    this.tblplaceholder = true;
    this.service.getcoursecategorylist(limit,index,searchkey, sorting).subscribe(res=>{
      this.tblplaceholder = false;
      this.courselist = res.data.data.course;
      this.courselist_length = res.data.data.totalcount;  
      this.dataSource = new MatTableDataSource<coursecategory>(this.courselist);
  ...
  updatestatus(id, status){
    ...
    this.service.updatestatuscourse(data).subscribe(res=>{

      this.toastr.success(this.i18n('Course Category status updated Successfully'), ''), {
        timeOut: 2000,
        closeButton: false,
      };
      this.getcourse(this.page,this.index, this.filterdata);
