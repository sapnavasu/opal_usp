...
export class GridlistComponent implements OnInit {
 ...
 status1;
  index = 0;
  dataSource : MatTableDataSource<educationlevel>;
  stktype;
  isfocalpoint;
  useraccess;
  createaccess = false;
  ...
  displayedColumns = [  
    { def: "name",search: "row-one", label: "Education Level", visible: true,disabled: false },
  ...
  ngOnInit(): void {
    ...
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][45] && this.useraccess[moduleid][45].create == 'Y'){
      this.createaccess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][45] && this.useraccess[moduleid][45].read == 'Y'){
      this.viewacess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][45] && this.useraccess[moduleid][45].update == 'Y'){
      this.updateaccess = true;
    }
    ...
    if (!this.viewacess) {
      ...
      // this.dataSource = new MatTableDataSource<educationlevel>(this.mockData);
      this.geteducationlevellist(this.page,this.index, this.filterdata);
      // this.getinitialdata();
  ...
  clearFilter(){
    ...
    $(".clear").trigger("click");   
    this.dataSource = new MatTableDataSource<educationlevel>([]);
    this.tblplaceholder = false;
    this.educationlist_length = 0;
   this.geteducationlevellist(this.paginator.pageSize, this.paginator.pageIndex, this.filterdata)
  ...
  add() {
    this.router.navigate(['/configuration/masterdataconfiguration/educationlevel/add']);
  }


  edit(id) {
    this.router.navigate(['/configuration/masterdataconfiguration/educationlevel/edit/'+id]);
  }

  geteducationlevellist(limit,index,searchkey, sorting = null){
    this.tblplaceholder = true;
    this.service.getreferencelist(this.mastertype,limit,index,searchkey, sorting).subscribe(res=>{
      this.tblplaceholder = false;
      this.educationlist = res.data.data.refer;
      this.educationlist_length = res.data.data.totalcount;  
      this.dataSource = new MatTableDataSource<educationlevel>(this.educationlist);
  ...
  sorting(event,key){
    ...
    var sorting = {
      ...
      this.geteducationlevellist(this.page,this.index, this.filterdata, sorting);
  }
  ...
  updatestatus(id, status){
    ...
    this.service.updatreferencestatus(data).subscribe(res=>{
      ...
      this.toastr.success(this.i18n('Education Level status updated Successfully'), ''), {
        timeOut: 2000,
        closeButton: false,
      };
      this.geteducationlevellist(this.page,this.index, this.filterdata);
