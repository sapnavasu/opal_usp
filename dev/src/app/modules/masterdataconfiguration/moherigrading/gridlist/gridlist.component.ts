...
export interface moherigrading {
  grade_en: any;
  status: any;
  createdOn: any;
  createdBy: any;
...
export class GridlistComponent implements OnInit {
 ...
 status1;
  index = 0;
  dataSource : MatTableDataSource<moherigrading>;
  stktype;
  isfocalpoint;
  useraccess;
  createaccess = false;
  ...
  mockData =[{grade_en:'Grade One', status:1, createdOn:'28-09-2023',
  createdBy:'Thomas',lastUpdatedOn:'28-09-2023',lastUpdatedBy:'Clare'}]
  displayedColumns = [  
    { def: "grade",search: "row-one", label: "MOHERI Grading", visible: true,disabled: false },
  ...
  ngOnInit(): void {
    ...
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][44] && this.useraccess[moduleid][44].create == 'Y'){
      this.createaccess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][44] && this.useraccess[moduleid][44].read == 'Y'){
      this.viewacess = true;
    }
    if(this.isfocalpoint != 1 && this.stktype == 1 && this.useraccess[moduleid] && this.useraccess[moduleid][44] && this.useraccess[moduleid][44].update == 'Y'){
      this.updateaccess = true;
    }
    ...
    if (!this.viewacess) {
      swal({
        ...
        // this.dataSource = new MatTableDataSource<moherigrading>(this.mockData);
        this.getmoherigrade(this.page,this.index, this.filterdata);
        // this.getinitialdata();
    }
  ...
  clearFilter(){
    ...
    $(".clear").trigger("click"); 
    this.dataSource = new MatTableDataSource<moherigrading>([]);
    this.tblplaceholder = false;
    this.gradelist_length = 0;
   this.getmoherigrade(this.paginator.pageSize, this.paginator.pageIndex, this.filterdata)
  ...
  add() {
    this.router.navigate(['/configuration/masterdataconfiguration/moherigrading/add']);
  }


  edit(id) {
    this.router.navigate(['/configuration/masterdataconfiguration/moherigrading/edit/'+id]);
  }

  getmoherigrade(limit,index,searchkey, sorting = null){
    this.tblplaceholder = true;
    this.service.getmoherigradelist(limit,index,searchkey, sorting).subscribe(res=>{
      this.tblplaceholder = false;
      this.gradelist = res.data.data.courses;
      this.gradelist_length = res.data.data.totalcount;  
      this.dataSource = new MatTableDataSource<moherigrading>(this.gradelist);
  ...
  sorting(event,key){
    ...
    this.getmoherigrade(this.page,this.index, this.filterdata, sorting);
  }
  ...
  updatestatus(id, status){
    ...
    this.service.updatmoherigradestatus(data).subscribe(res=>{

      this.toastr.success(this.i18n('Moheri Grade status updated Successfully'), ''), {
        timeOut: 2000,
        closeButton: false,
      };
      this.getmoherigrade(this.page,this.index, this.filterdata);
