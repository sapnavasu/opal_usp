...
class MastercompanyprofileController extends MasterController
{
    ...
    public function actionContactusmasterdata(){
       $returnData = [];
        ...
        try{
            ...
            if(empty($cache->retreive($cacheKey))){
                $cacheQuery = \app\models\OpalcontactquerymstTbl::contactQueryCache();
                $masterData = \app\models\OpalcontactquerymstTbl::find()->where(['ocqm_status'=>1])->asArray()->all();
       ...
       if($masterData != ''){
           foreach($masterData as $index => $value){ 
               $returnData[$index]['value'] = $value['opalcontactquerymst_pk'];
               $returnData[$index]['viewValue'] = $value['ocqm_contactquery'];
           }
            $response['msg'] = 'success';
            $response['status'] = 1;
   ...
   public function actionContactusccdata(){  
       ...
       if(in_array($_REQUEST['apiFor'],['and','ios']) && !empty($_REQUEST['emailstart']))
       {
        ...
        try{
            ...
            if(empty($cache->retreive($cacheKey))){
                $cacheQuery =  \app\models\OpalusermstTbl::getUserMstCacheQuery();
                $masterData = \app\models\OpalusermstTbl::find()->where('oum_opalmemberregmst_fk=:fk and UM_Status =:sts and oum_emailconfirmstatus =:stsemail and opalusermst_pk !=:usrfk '.$addlikeemail.'',['sts'=>'A','stsemail'=>1,':fk'=> $regpk,':usrfk'=> $userPk])->all();
                $cache->store($cacheKey, $masterData, $duration = 0 , $cacheQuery);
            } else {
                $masterData = $cache->retreive($cacheKey);
            }
            ...
            $masterData = \app\models\OpalusermstTbl::find()->where('oum_opalmemberregmst_fk=:fk and oum_status =:sts and oum_emailconfirmstatus =:stsemail and opalusermst_pk !=:usrfk',['sts'=>'A','stsemail'=>1,':fk'=> $regpk,':usrfk'=> $userPk])->all();
        }
      
       if(count($masterData) > 0){
           $edata = array_column($masterData,'oum_emailid' );
        }
        $response['msg'] = 'success';
        $response['status'] = 1;
        $response['data'] = $edata;
   ...
   public function actionInsertcontactus(){
       ...
       $cmpPK = \yii\db\ActiveRecord::getTokenData('oum_opalmemberregmst_fk',true);
    ...
    if(!empty($cmpPK)){
     ...
     //  if(1)
       { 
            ...
            $contactusObj = new \app\models\OpalcontactusdtlsTbl;
            $contactusObj->ocud_conttype =2;
            $contactusObj->ocud_companyname=$_REQUEST['companyName'];
            $contactusObj->ocud_username=$_REQUEST['personName'];
            $contactusObj->ocud_emailid=$_REQUEST['emailId'];
            $contactusObj->ocud_emailcc= 'karan@businessgateways.com'; //$ccemailstr;
            $contactusObj->ocud_opalcontactquerymst_fk=$_REQUEST['typeofQuery'];
            $contactusObj->ocud_message=$_REQUEST['about'];
            $contactusObj->ocud_subject=$_REQUEST['subJect'];


            $fileDetails=\app\models\OpalmemcompfiledtlsTbl::findOne($cmpPK);
            $filedtlpk=Yii::$app->db->createCommand("select `omcfd_opalmemcompfiledtls_pk` FROM `opalmemcompfiledtls_tbl` WHERE `omcfd_opalmemberregmst_fk`= $cmpPK and `omcfd_uploadedby`=$userPk and `omcfd_filemst_fk`=4 and `omcfd_isdeleted` = 0 and cast(`omcfd_uploadedon` as Date) = cast(Date(Now()) as Date)")->queryAll();
         
            foreach($filedtlpk as $val)
            { 
                $memfiledtlpk  .= trim($val['omcfd_opalmemcompfiledtls_pk'].',');
                $resfiledtlpk = rtrim($memfiledtlpk,',');
            }  
           
            $contactusObj->ocud_opalmemcompfiledtls_fk=$resfiledtlpk;
            $contactusObj->ocud_opalmemberregmst_fk = $cmpPK;
            $contactusObj->ocud_opalusermst_fk = $userPk;
            $contactusObj->ocud_createdon=date('Y-m-d H:i:s');
            $contactusObj->ocud_mailtemplatepath = "test";
       }
       ...
       if($data != ''){
           ...
           $contactusObj = new \app\models\OpalcontactusdtlsTbl;
           $contactusObj->ocud_conttype =2;
           $contactusObj->ocud_companyname=$data['contdata']['contactUs']['companyName'];
           $contactusObj->ocud_username=$data['contdata']['contactUs']['personName'];
           $contactusObj->ocud_emailcc= $ccemailstr;
           $contactusObj->ocud_emailid=$data['contdata']['contactUs']['emailId'];
           $contactusObj->ocud_opalcontactquerymst_fk=$data['contdata']['contactUs']['typeofQuery'];
           $contactusObj->ocud_message=$data['contdata']['contactUs']['about'];
           $contactusObj->ocud_subject=$data['contdata']['contactUs']['subJect'];
           $contactusObj->ocud_opalmemcompfiledtls_fk=implode(",",$data['contdata']['contactUs']['contactusdoc']);
          // $contactusObj->ocud_opalmemcompfiledtls_fk=$resfiledtlpk;
           $contactusObj->ocud_opalmemberregmst_fk = $cmpPK;
           $contactusObj->ocud_opalusermst_fk = $userPk;
           $contactusObj->ocud_createdon=date('Y-m-d H:i:s');
           $contactusObj->ocud_mailtemplatepath = "test";
            ...
            if($contactusObj->save()){
              ...
              //$changethis =badmashree@businessgateways.com;
               $email_to ='pavithra.b@businessgateways.com';
               $id = Yii::$app->db->getLastInsertID();
     
               if(!empty($contactusObj->ocud_emailcc)){
                    //with CC mail
                   ...
                   $url = $baseUrl."api/ma/mail/sendmail";
                    ...
                    $_data1=[
                        ...
                        'type'=>'contactus',
                        'userpk' =>$id,
                        'mailcc' =>$contactusObj->ocud_emailcc
                    ];
