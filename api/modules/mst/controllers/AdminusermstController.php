...
use api\modules\mst\models\AdminusermstTbl;

class AdminusermstController extends MasterController
{
    public $modelClass = 'api\modules\mst\models\AdminusermstTbl';
    ...
    public function actionIndex(){
        ...
        $query->select(['adminusermst_tbl.*',
            'if(adminusermst_tbl.aum_status = "A", "primary","warn") as `color`',
            'adminuserrolemst_tbl.aurm_userrole']);
    ...
    public function actionView($id){
        $country = AdminusermstTbl::find()->where([
            'adminusermst_pk'    =>  $id
        ])->one();
       
        //echo "<pre>";print_r($AdminusermstTbl);die;
    ...
    public function actionSearch($id){
        $country = AdminusermstTbl::find()->active()->where([
            'adminusermst_pk'    =>  $id
        ])->one();
        if($country){
            return $country;
    ...
    public function actionNewuser(){
        ...
        if ($model->validate() && $model->save()) {
            $result=array(
                ...
                'returndata' => $model->adminusermst_pk
            );
        } else {
            $result=array(
                'status' => 422,
    ...
    public function actionUpdate($id) {
        ...
        $request_body	=	file_get_contents('php://input');
        $data =	json_decode($request_body, true);
        $model = AdminusermstTbl::find()->where([
            'adminusermst_pk'    =>  $id
        ])->one();
        if($data['usermaster'])
        {
        ...
        {
            $model = AdminusermstTbl::find()->where([
                'adminusermst_pk'    =>  $data['updatestatus']
            ])->one();
