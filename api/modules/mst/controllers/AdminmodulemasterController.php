...
use api\modules\mst\models\AdminmodulemstTb;
use yii\web\Response;
use yii\db\Query;

class AdminmodulemasterController extends MasterController
{
    public $modelClass = 'api\modules\mst\models\AdminmodulemstTb';
    ...
    public function actionView($id){
        $module = AdminmodulemstTb::find()->where([
            'AdminModuleMst_Pk'    =>  $id
        ])->one();
        if($module){
            return $module;
        } else {
    ...
    public function actionIndex(){
        ...
        $query->select(["`adminmodulemst_tbl`.`*`, `userrolemst_tbl`.`URM_Role`",
            "if(`adminmodulemst_tbl`.`AMM_Status` = 'A', 'primary','warn') as `color`"]);
        $query->leftJoin('userrolemst_tbl', 'userrolemst_tbl.UserRoleMst_Pk = adminmodulemst_tbl.AMM_UserRoleMst_Fk');
        $query->asArray();
    ...
    public function actionUpdate($id) {
        ...
        {
            $model = AdminmodulemstTb::find()->where([
                'AdminModuleMst_Pk'    =>  $id
            ])->one();
            foreach ($data['modulemaster'] as $key=>$value) {
                if($key=="Status")
        ...
        {
            $model = AdminmodulemstTb::find()->where([
                'AdminModuleMst_Pk'    =>  $data['updatestatus']
            ])->one();
    ...
    public function actionGetadminmodule(){
        $connection = \Yii::$app->db;
        $model = $connection->createCommand('SELECT AdminModuleMst_Pk,AMM_ModName FROM adminmodulemst_tb WHERE 	AMM_Status="A"');
        $users = $model->queryAll();
        return $users;
    }
