...
use api\components\Accountsettings;
...
class AccountsettingsController extends NbfMasterController
{
    ...
    */
    ...
    public function actionAccsettingsdata() {
        ...
        $accSettingsDtl = Accountsettings::getAccountSettingsData($regpk, $userPk, $evp);
    ...
    public function actionSavelogo() {
        ...
        $request_body = file_get_contents('php://input');
        $data = json_decode($request_body, true);
        $accSettingsDtl = Accountsettings::changeCompanyOrProfileLogo($data['filePk'], $userType);
        $msg['msg'] = 'failure';
        $msg['status'] = 0;
        if($accSettingsDtl){
            $msg['msg'] = 'success';
    ...
    public function actionSaveuserdp() {
//        $userType = \yii\db\ActiveRecord::getTokenData('user_type', true);
        ...
        $data = json_decode($data['filePk'],true);
        $accSettingsDtl = null;
        if($data)
        {
           $accSettingsDtl = \app\models\OpalmemcompfiledtlsTbl::SaveUserDp($data, $userPk);
        }
        
        $model = \app\models\OpalusermstTbl::findOne($userPk);
        $model->oum_userdp = $accSettingsDtl;
        if(!$model->save())
        {
            echo "<pre>";
        ...
        if($accSettingsDtl){
            $msg['msg'] = 'success';
            $msg['status'] = 1;
            $msg['data'] = $accSettingsDtl;
        }
        return $this->asJson($msg);
    }
    
    public function actionSaveuserdtls() {
        $request_body = file_get_contents('php://input');
        $data = json_decode($request_body, true);
        $userPk = \yii\db\ActiveRecord::getTokenData('opalusermst_pk', true);
        $valid = Accountsettings::ValidateEditDtlsInputs($data['form'], $userPk);
        $accSettingsDtl = false;
        if($valid)
        {
             $accSettingsDtl = Accountsettings::SaveUserDtls($data['form'], $userPk);
        }
    ...
    public function actionChangeuser(){
       ...
       if($ischangeuser == 0){
            ...
            Accountsettings::generateChangeUserAuthorizeMail($data['userPk'], $data['newAdminUserPk']);           
       }else{
           $msg['msg'] = 'success';
           $msg['status'] = 2;
       }        
    ...
    public function actionChangeuserauthorize(){
        ...
        $type = $data['catype'];
        $time = $data['t'];
        $authorized = Accountsettings::changeUserAuthorize($encryptedCurrentAdminPk,$encryptedNewAdminPk,$type,$time);
