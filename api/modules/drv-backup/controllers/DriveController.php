...
class DriveController extends MasterController
{
     ...
     public function behaviors()
    {
        ...
        $behaviors['contentNegotiator'] = [
            'only'=>['list','add','filemst','upload','datatoimage','mapreference','removereference','removefile'],
    ...
    public function actionList($directory)
    {
       //  print_r($directory); 
        ...
        $addedToday= \common\models\MemcompfiledtlsTbl::addedToday();
        $totalUploads= \common\models\MemcompfiledtlsTbl::totalUploads();
	...
	public function actionFilemst()
    {
		...
		$fileTemplate= \app\models\OpalfileuploadmstTbl::find()->filee($resParam->fileRefNo);
    ...
    {   
         ...
         $filename = $_REQUEST['file_name'];  
		  $fileTemplate= \app\models\OpalfileuploadmstTbl::find()->filee($key);
        if (!empty($_REQUEST['key'])) {

			
         if($_REQUEST['key'] == 4 || $_REQUEST['key'] == 5){
          ...
          !empty($fileTemplate) ? $upload = Common::file_upload_temp($fileTemplate, $_FILES, false,'') : $upload = ["msg" => "No data exists towards the key", "status" => 0];   
   ...
   public function actionUploadcrop()
    ...
    public function actionMapreference()
    {
        $returnStatus=[];
			...
			foreach($selectedPks as $pk){
                $fileDetails= \app\models\OpalmemcompfiledtlsTbl::findOne($pk);
                $referrenceId=[];
                if(empty($fileDetails->omcfd_referredin)){
                    $referrenceId[]=$fileMstPk;
                }else{
                    $referrenceId=json_decode($fileDetails->omcfd_referredin);
                    $referrenceId[]=$fileMstPk;
                }
                $referrenceId=array_unique($referrenceId);
                $referrenceId=json_encode($referrenceId);
                $fileDetails->omcfd_referredin=$referrenceId;
                $fileDetails->update();
    ...
    public function actionRemovereference()
    {
        $returnStatus=[];
            ...
            $fileDetails=\app\models\OpalmemcompfiledtlsTbl::findOne($selectedPk);
            $referrenceId=[];
            if(!empty($fileDetails->omcfd_referredin)){            
                $referrenceId=json_decode($fileDetails->omcfd_referredin);
     ...
     public function actionView()
    {
	  ...
	  $trackVisit=Security::sanitizeInput(Security::decrypt($_REQUEST['t']), "number");
		  ...
		  $fileDetails= \app\models\OpalmemcompfiledtlsTbl::find()->where('omcfd_opalmemcompfiledtls_pk=:filedtlspk',[':filedtlspk'=>$fileDtlsPk])->one();
                  
		if($_REQUEST['dbug']==1){
			echo '<pre>';
			print_r($fileDetails);
			print_r($fileDetails->omcfdFilemstFk);
			exit;
		}
		// print_r($fileDetails);exit;
		 $directory=$fileDetails->omcfdFilemstFk->ofm_phyfilepath;
        $srcDirectory=Yii::$app->params['srcDirectory'];
        $userDirectory="comp_".$fileDetails->omcfd_opalmemberregmst_fk."/user_".$fileDetails->omcfd_uploadedby;
        $uploadPath=  \Yii::$app->params['uploadPath'];
        $originalImageDirectory = $srcDirectory.$uploadPath."/".$userDirectory. '/' . $directory.'/';
     
		  $download_path = $originalImageDirectory;  
		 $file = $fileDetails->omcfd_sysgenerfilename;  
		if($trackVisit){
			$url = Yii::$app->request->baseUrl."/".Yii::$app->request->pathInfo;
			$descText="Viewed the {$fileDetails->omcfdFilemstFk->ofm_filelabel}, {$fileDetails->omcfd_origfilename}.";
//			\common\components\UserActivityLog::logUserActivity(5,$descText,$url,$fileDetails->omcfdFilemstFk->fm_modulemst_fk,$userPk,$companyPk);
		}
