...
class ProductFilterQueryGen extends Querybuilder
{
    ...
    */
        ...
        if (!empty($this->filterJoin)) {
            foreach ($this->filterJoin as $key => $data) {
                if (isset($this->joinArr[$key])) {
                    ...
                    if(!in_array($key,['psector','tpsector']))
            ...
            switch ($dbFieldColKey) {
                ...
                case 'psector':
                    if ($this->tableConditionType == 'T'){
                        // $this->filterJoin['tpsector_secmap'] = 1;
                        $this->filterJoin['tpsector'] = 1;
                    }else{
                        $this->filterJoin['psector'] = 1;
                        // $this->filterJoin['psector'] = 1;
                    }
    ...
    {
        $ProductArr = $data['product']['dataVal'];
            ...
            foreach ($ProductFilterInputArr as $key => $list) {
                ...
                if (in_array($this->tableConditionType, ['B', 'T'])) {
                    $lis[] = array_merge([$this->AND], [ArrayHelper::map($list, function ($element) {
                        $DbColArr = [1 => 'mcprd_bgiindcodecateg_fk', 2 => 'mcprd_bgiindcodesubcateg_fk', 3 => 'mcprd_bgiinduscodeprodmst_fk', 4 => 'mcprd_productmst_fk'];
                        return $DbColArr[$element['uid']];
                    }, 'id')]);
                }
                if (in_array($this->tableConditionType, ['B', 'M'])) {
                    $lis[] = array_merge([$this->AND], [ArrayHelper::map($list, function ($element) {
                        $DbColArr = [1 => 'mcprdm_bgiindcodecateg_fk', 2 => 'mcprdm_bgiindcodesubcateg_fk', 3 => 'mcprdm_bgiinduscodeprodmst_fk', 4 => 'mcprdm_productmst_fk'];
                        return $DbColArr[$element['uid']];
                    }, 'id')]);
                }
            }
