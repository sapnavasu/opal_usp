...
class Servicefilter{
            ...
            case 'businessUnitFilter':
                switch ($typeKey) {
                    case '1':
                        $result = [
                            'type'=>1,
                            'fieldName'=>'secm.SecM_SectorName',
                            'combination'=>self::fieldCombination(),
                            'dataType'=>self::fieldDataType()
                        ];
                        break;
    ...
    public static function organizeFilter($filterType, $searchType){
        switch ($filterType) {
            ...
            case '3': // Market Presence Filter // Completed
                ...
                break;
            case '4': // Service Filter
                $filterResult = self::servicesFilter($searchType);
                break;
    ...
    public function servicesFilter($searchType){
        switch ($searchType) {
            case '2': // Products
                $combinationArr = [
                    ['type'=>'1','name'=>'Equals to','displayType'=>'1'],
    ...
    */
    ...
    public static function organizeFilterData($filterType, $searchType){
        switch ($filterType) {
            ...
            case '3': // Market Presence Filter
                ...
                break;
            case '4': // Service Filter
                $filterResult = self::servicesFilterData($searchType);
                break;
    ...
    public function organizeUserFilterData($searchType){
        ...
        switch ($searchType) {
            case '1': // Department Details
                $filterDataArr = Bizsearch::getServiceDepartment($cmpPK);
                break;
            case '2':
                $filterDataArr = Bizsearch::getServiceDesignationLevel($cmpPK);
                break;
            case '3': // User Status
                $filterDataArr = [
    ...
    public function bsourceFilterData($searchType){
        ...
        switch ($searchType) {
            case '1': // Business Source
                $filterDataArr = Bizsearch::getServiceBusinessSource($cmpPK);
                break;
        }
        return $filterDataArr;
    }

    public function servicesFilterData($searchType){
        $regPK = \yii\db\ActiveRecord::getTokenData('reg_pk',true);
        $cmpPK = \yii\db\ActiveRecord::getTokenData('comp_pk',true);
        ...
        switch ($searchType) {
            case '2': // National Product
                ...
                break;
            case '3': // Contact Person
                $filterDataArr = Bizsearch::getServiceUsers($cmpPK);
                break;
        }
        return $filterDataArr;
    }
