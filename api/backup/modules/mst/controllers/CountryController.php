...
class CountryController extends MasterController
{
    ...
    public function actionCountrylist()
    {
        ...
        try{
            ...
            $query = CountryMasterQuery::getCountryListwithDialCodeCache();
            if(empty($cache->retreive($cacheKey))){
                if($type == 'ex-oman')
                {
                   $data = CountryMasterQuery::getCountryListwithDialCode( $deployCountry,$type );  
                }
                else if($type == 'oman-only')
                {
                   $data = CountryMasterQuery::getCountryListwithDialCode( $deployCountry,$type );  
                }
                else
                {
                   $data = CountryMasterQuery::getCountryListwithDialCode( $deployCountry );
                }
