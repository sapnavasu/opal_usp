...
class Configuration extends \yii\base\BaseObject
{
    ...
    public const FOLDER_DIRECTORY = (__DIR__) . '/../../backend/json/';
    public const FOLDER_DIRECTORY_CONTENT = (__DIR__) . '/../../backend/json/content/';


    public static function json($newarray, $filename)
    {
    ...
    public static function jsonkeyval($newarray, $filename,$foldername)
    {
        $path = self::FOLDER_DIRECTORY_CONTENT;
        $path = $path.$foldername.'/';
        if (!is_dir($path)) {
            mkdir($path, 0777, true);
        }
        $new_arr = [];
        foreach($newarray['socialName'] as $key=>$val){
            $new_arr[$val] = $newarray['socialUrl'][$key];
        }
        $fp = fopen($path . $filename . '.json', 'w');
        fwrite($fp, json_encode($new_arr));
    }
    ...
    public static function getjsonforcontent($filename,$folder)
    {
        $path = self::FOLDER_DIRECTORY_CONTENT;
        $path = $path.$folder.'/';
        if (!is_dir($path)) {
    ...
    public static function getjsonDataApplication($filename, $element = null)
    {
        $path = self::FOLDER_DIRECTORY;
        if (!is_dir($path)) {
            mkdir($path, 0777, true);
    ...
    public static function getfilepath($filename, $element, $field)
    {
        $path = self::FOLDER_DIRECTORY;
        if (!is_dir($path)) {
            mkdir($path, 0777, true);
    ...
    public static function getAllJsonData()
    {
    ...
    public static function getProductjsonData($jsondata)
    {
...
//                    'XOF' => '',
    ...
    public static function getEncryptedUserKey($joinKey='**j2**') {
        $userPk= \yii\db\ActiveRecord::getTokenData('UserMst_Pk', true);    
        $usrmst = \common\models\UsermstTbl::find()->where('UserMst_Pk=:userPk',[':userPk' => $userPk])->one();
        if(!empty($usrmst)){
            $userName = $usrmst['UM_LoginId'];
            $pass = $usrmst['UM_Password'];
            $key = $userName.$joinKey.$pass;
            return \common\components\Security::aesencrypt($key);
        }else echo'user Not found';
    }


    public static function getDecryptedUserKey($key,$joinKey='**j2**') {
        $deKey = \common\components\Security::aesencrypt($key);
        $unkey = explode($joinKey,$deKey);
        $userPk= \yii\db\ActiveRecord::getTokenData('UserMst_Pk', true);    
        $usrmst = \common\models\UsermstTbl::find()->where('UserMst_Pk=:userPk',[':userPk' => $userPk])->one();
        if(!empty($usrmst)){
            $userName = $usrmst['UM_LoginId'];
            $pass = $usrmst['UM_Password'];
            if($userName == $unkey[0] && $pass == $unkey[1]){
                return ['status'=>1,'userObj'=>$usrmst];
            }else{
                return ['status'=>0,'Status_Code'=>'JSRS002','msg'=>'User is not matching with Current login'];
            }
        }else return ['status'=>0,'Status_Code'=>'JSRS001','msg'=>'User not Found'];
    }
