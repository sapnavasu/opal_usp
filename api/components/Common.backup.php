...
//        $userPk = ActiveRecord::getTokenData('UserMst_Pk', true);
    ...
    $isSameFileExist = \common\models\MemcompfiledtlsTbl::find()->where(
                    'mcfd_filemst_fk=:mstpk AND '
                    . 'mcfd_memcompmst_fk=:comppk AND '
    ...
    if (in_array($file_extension, $allowedExtensions) || 1) { //For Extension Validation
        if (!empty($file['file']) && Common::mimeCheck($file['file'])  || 1) {
            if ($allowedFileSize >= $file_size  || 1) {
                ...
                move_uploaded_file($tempPath, $target_path);
                $uploadData['fileUrl'] = $linkUrl;
                $upload = \common\models\MemcompfiledtlsTbl::singleUpload($uploadData, $target_path);
                 return $upload;
            } else {
                return [
                    "msg" => "This file exceeds the maximum size allocated",
    ...
    public static function generateLyPISID($regpk,$countryPk,$compPk) {
        $projectNameConfig  =   \Yii::$app->params['thisProjectFor'];
        $projectConfigArr   =   \Yii::$app->params['projectConfig'];
...
//            $n_format = round($n / 1000);
    ...
    public static function mimeCheck($fname) {
        $mime = mime_content_type($fname["tmp_name"]);
        $pathinfo = pathinfo($fname["name"]);
        $ext = strtolower($pathinfo['extension']);
        $mime_typ = array(
            'png' => ['image/png'],
            'jpe' => ['image/jpeg'],
