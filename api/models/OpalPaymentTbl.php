 ...
 * @property string $apppdt_pymtproof
 ...
 */
class OpalPaymentTbl extends \yii\db\ActiveRecord
{
    /**
     * {@inheritdoc}
     */
    ...
    public function rules()
    {
        return [
            ...
            [['apppdt_opalmemberregmst_fk','apppdt_apppytminvoicedtls_fk','apppdt_applicationdtlstmp_fk', 'apppdt_paymenttype', 'apppdt_paymentmode', 'apppdt_currency', 'apppdt_requesttype', 'apppdt_opalusermst_fk', 'apppdt_createdby', 'apppdt_updatedby', 'apppdt_status', 'apppdt_appdecby'], 'integer'],
    ...
    public static function savePayment($requestdata){
        $model = new OpalPaymentTbl();
        $model->apppdt_opalmemberregmst_fk = $requestdata['apppdt_opalmemberregmst_fk'];
        $model->apppdt_apppytminvoicedtls_fk = $requestdata['apppdt_apppytminvoicedtls_fk'];
        $model->apppdt_applicationdtlstmp_fk = $requestdata['apppdt_applicationdtlstmp_fk'];
        $model->apppdt_currency = $requestdata['apppdt_currency'];
        $model->apppdt_amount = $requestdata['apppdt_amount'];
        $model->apppdt_vatchrgs = $requestdata['apppdt_vatchrgs'];
        $model->apppdt_vatpercent = $requestdata['apppdt_vatpercent'];
        $model->apppdt_requesttype = $requestdata['apppdt_requesttype'];
        $model->apppdt_status = 1;
        $model->apppdt_createdon = date("Y-m-d H:i:s");
        $model->apppdt_createdby = $requestdata['apppdt_createdby'];
        $model->apppdt_noofstaffeval = $requestdata['apppdt_noofstaffeval'];
        $model->apppdt_staffevafee = $requestdata['apppdt_staffevalfee'];
       // $model->apppdt_staffevalfee = $requestdata['apppdt_staffevalfee'];
        if($model->save()){
            return $model->apppymtdtlstmp_pk;
        } else {
            echo "<pre>";
    ...
    public static function updatePayment($requestdata){
        ...
        $model = OpalPaymentTbl::find()->where(['apppymtdtlstmp_pk' => $requestdata['apppymtdtlstmp_pk']])->one();
        if($model){
        $model->apppdt_opalmemberregmst_fk = $requestdata['apppdt_opalmemberregmst_fk'];
        $model->apppdt_applicationdtlstmp_fk = $requestdata['apppdt_applicationdtlstmp_fk'];
        $model->apppdt_currency = $requestdata['apppdt_currency'];
