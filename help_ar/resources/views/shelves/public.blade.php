...
<h5> Extended Help Page </h5>
    @foreach($masterData['shelves'] as $keyshelf => $shelve)
    @if($shelve['classification']==1)
    <div data-level1="level1_<?php echo $keyshelf; ?>" class="tab first-child">
    <div class="tab-title">
         {{$shelve['name']}}
    </div>
    </div>    
        ...
        <div class="description"> <?php echo $shelve['description'] ?></div>
            @if($shelve['column_level']==2)  
                @foreach($shelve['books'] as $keybook => $book)
                    <div class="tablevelone first-child">
                        <a class="tab-title_level1" id="targetbksegment_<?php echo $book['specid']; ?>">{{$book['name']}} </a>
                    </div>    
                    <div class="tab-content">
                        <div class="customalignone">
                            ...
                            @foreach($book['chapters'] as $keya => $chapter)
                                <div class="tableveltwo first-child">
                                    <a class="tab-title_level2" id="targetcpsegment_<?php echo $chapter['chapid']; ?>"> {{$chapter['name']}}</a>
                                </div>    
                                <div class="tab-content">
                                    <div class="customaligntwo">
                                        <div class="descriptiontwo"> <?php echo $chapter['description'] ?></div>
                                            ...
                                            <div class="wrapper">
                                                ...
                                                <div id="target_page<?php echo $keyb; ?>"  class="accordion__panel">
                                                   <div class="accordion__heading"><button class="accordion__btn_1" >{{$page['name']}}</button><button class="copylinkbutton" id="copyButton" onclick="copyLink(<?php echo $keyb; ?>)" style="text-align: right;">Copy Link</button> <span id="copyResult"></span></div>
                                                    <div class="accordion__content" data-contenttype="level2" id="targetpage_<?php echo $keyb; ?>"  data-bookkey="<?php echo $keyshelf; ?>" data-segmentkey="<?php echo $book['specid']; ?>" data-chapterkey="<?php echo $chapter['chapid']; ?>" data-pagekey="<?php echo $keyb; ?>">
                    ...
                    <div class="wrapper">
                                <div class="accordion">
                                    <div class="accordion__panel">
                                        <div class="accordion__heading"><button class="accordion__btn" >{{$page['name']}}</button> <button class="copylinkbutton" id="copyButton" onclick="copyLink(<?php echo $keyb; ?>)" style="text-align: right;">Copy Link</button> <span id="copyResult"></span> </div>
...
let slideToggle = (target, duration = 500) => {
 ...
 $(document).ready(function(){
    ...
    var chapterkey=$("#targetpage_<?php echo $target; ?>").attr('data-chapterkey');
